...
const MAX_JOINS_PER_WINDOW = 500;
...
export default class ChannelManager {
	...
	async #joinWorker(c) {
		if (this.anon.joinedChannels.has(c)) return;
		await this.joinRateLimiter.wait();
		logger.debug(`[ChannelManager] trying to join ${c}`);
		this.anon
			.join(c)
			.catch(err => logger.error(`[ChannelManager] failed to join ${c}:`, err));
	}
	...
	async part(c) {
		...
		this.joinQueue.removeMatching(item => item === c);
		try {
